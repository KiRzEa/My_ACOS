...
class TwoBertPreTrainedModel(nn.Module):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained models.
    """
    def __init__(self, config, *inputs, **kwargs):
        super(TwoBertPreTrainedModel, self).__init__()
        if not isinstance(config, BertConfig):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class `BertConfig`. "
                "To create a model from a Google pretrained model use "
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, ae_path, *inputs, **kwargs):
        ...
        ae_archive_file = os.path.join(ae_path, WEIGHTS_NAME)
        ae_resolved_archive_file = cached_path(ae_archive_file, cache_dir=cache_dir)
        ae_state_dict = torch.load(ae_resolved_archive_file, map_location='cpu')
        ...
        for key in list(ae_state_dict.keys()):
            ae_state_dict['ae_'+key] = ae_state_dict.pop(key)
        old_keys = []
        new_keys = []
        for key in ae_state_dict.keys():
            ...
            #     print('ae_state_dict',ae_state_dict[key])
            if new_key:
                old_keys.append(key)
                new_keys.append(new_key)
        for old_key, new_key in zip(old_keys, new_keys):
            ae_state_dict[new_key] = ae_state_dict.pop(old_key)

        if state_dict is None and not from_tf:
            state_dict = torch.load(resolved_archive_file, map_location='cpu')
        state_dict.update(ae_state_dict)
        del ae_state_dict
        if from_tf:
            # Directly load from a TensorFlow checkpoint
            return load_tf_weights_in_bert(model, weights_path)
    ...
    def forward(self, inputs, seq_len=None, out_type='all'):
        ...
        * out_type [str]: use 'all'/'last' to choose
        ...
        if seq_len is not None:
            ...
            sort_seq_len = torch.index_select(sort_seq_len, 0, torch.nonzero(sort_seq_len, as_tuple=False).contiguous().view(-1))
...
class BertForQuadABSA(BertPreTrainedModel):

    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False):
        super(BertForQuadABSA, self).__init__(config)
